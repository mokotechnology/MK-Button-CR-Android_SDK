...
import com.moko.support.cr.handler.MokoCharacteristicHandler;
...
public class CRMokoSupport extends MokoBleLib {
    private HashMap<OrderCHAR, BluetoothGattCharacteristic> mCharacteristicMap;

    private static volatile CRMokoSupport INSTANCE;

    private Context mContext;

    private MokoBleConfig mBleConfig;

    private CRMokoSupport() {
        //no instance
    }

    public static CRMokoSupport getInstance() {
        if (INSTANCE == null) {
            synchronized (CRMokoSupport.class) {
                if (INSTANCE == null) {
                    INSTANCE = new CRMokoSupport();
                }
            }
        }
        return INSTANCE;
